...
while [ $CURRENT_BYTES -lt $TOTAL_BYTES ]; do
    ...
    BLOCK_SIZE=10485760  # 10MB per block
    WRITTEN=0
    > "$FILE_PATH"
    ...
    while [ $WRITTEN -lt $FILE_SIZE ]; do
        ...
        # Generate random readable text
        TEMP_DATA=$(mktemp)
        head -c $CHUNK /dev/urandom | tr -dc '[:print:]\n' > "$TEMP_DATA"

        # Check how much was actually written
        ACTUAL=$(stat -c%s "$TEMP_DATA")

        # If it's smaller (due to filtering), pad it
...
# 2# Generate readable data fast
    # head -c $FILE_SIZE /dev/urandom | tr -dc '[:print:]\n' > "$FILE_PATH"
    # # Pad if smaller than expected
    # ACTUAL_SIZE=$(stat -c%s "$FILE_PATH")
    # if [ $ACTUAL_SIZE -lt $FILE_SIZE ]; then
    #     PADDING=$((FILE_SIZE - ACTUAL_SIZE))
    #     head -c $PADDING < /dev/zero | tr '\0' ' ' >> "$FILE_PATH"
    # fi
    
# 3# 384mb generated instead of 1024mb
    # dd if=/dev/urandom bs=1M count=$((FILE_SIZE / 1048576)) 2>/dev/null | tr -dc '[:print:]\n' > "$FILE_PATH"
    # REMAINDER=$(( FILE_SIZE % 1048576 ))
    # if [ $REMAINDER -gt 0 ]; then
    #     dd if=/dev/urandom bs=1 count=$REMAINDER 2>/dev/null | tr -dc '[:print:]\n' >> "$FILE_PATH"
    # fi

    CURRENT_BYTES=$((CURRENT_BYTES + FILE_SIZE))
    FILE_COUNT=$((FILE_COUNT + 1))
#  echo "Created $FILE_PATH (~$FILE_SIZE bytes)" #shows full path
    printf "\nCreated %-20s (%d bytes)\n" "$FILE_NAME.$FILE_EXT" "$FILE_SIZE" #shows only file's name
...
echo "Finished! Created $FILE_COUNT readable files totaling $CURRENT_BYTES bytes in $OUTPUT_DIR."
